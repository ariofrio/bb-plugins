...
test("sorts independent plugin releases by id", () => {
  const plan = buildReleasePlan([
    {
      directory: "plugins/bb-plugin-thread-tasks",
      before: plugin("bb-plugin-thread-tasks", "0.5.0", "Thread tasks"),
      after: plugin("bb-plugin-thread-tasks", "0.6.0", "Thread tasks"),
    },
    {
  ...
  assert.deepEqual(
    ...
    [
      ...
      { id: "thread-tasks", tag: "thread-tasks/v0.6.0" },
    ],
  );
});
  ...
  assert.throws(
    () =>
      buildReleasePlan([
        {
          directory: "plugins/bb-plugin-thread-tasks",
          before: plugin("bb-plugin-thread-tasks", "0.5.0"),
          after: plugin("bb-plugin-thread-tasks", "0.4.0"),
        },
      ]),
